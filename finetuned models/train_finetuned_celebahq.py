...
class DDPMTrainer:
    def __init__(
        ...
        device,
        num_epochs=50,
        save_dir='ddpm_checkpoints_celebahq',
        ...
        self.vis_dir = self.save_dir / 'visualizations_celebahq'
    ...
    def save_checkpoint(self, epoch, is_best=False):
        ...
        checkpoint_path = self.save_dir / f'checkpoint_epoch_{epoch}_celebahq.pt'
        torch.save(checkpoint, checkpoint_path)
        
        if is_best:
            best_model_path = self.save_dir / 'best_model_celebahq.pt'
    ...
    def create_prediction_visualization(self, mri, pet, pred_pets, epoch):
        try:
            ...
            fig.suptitle(f'Predictions at Epoch {epoch} (CelebAHQ)', fontsize=16, y=0.92)
            ...
            plt.tight_layout()
            save_path = self.vis_dir / f'predictions_epoch_{epoch:04d}_celebahq.png'
    ...
    def plot_training_curves(self):
        ...
        try:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title('Training Loss Over Time (CelebAHQ)')
            plt.grid(True)
            plt.legend()
            ...
            plt.title('Image Quality Metrics Over Time (CelebAHQ)')
            plt.grid(True)
            plt.legend()
            
            plt.tight_layout()
            save_path = self.save_dir / 'training_metrics_celebahq.png'
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
            plt.close()
    ...
    def create_final_evaluation_visualization(self, num_samples=10):
        ...
        """
        save_path = self.save_dir / 'final_evaluation_results_celebahq.png'
        self.model.eval()
        
        # Create figure with 3 columns (MRI, Real PET, Predicted PET) and num_samples rows
        fig, axes = plt.subplots(num_samples, 3, figsize=(15, 5 * num_samples))
        fig.suptitle('Model Evaluation: MRI to PET Translation (CelebAHQ)', fontsize=16, y=0.92)
...
if __name__ == "__main__":
    ...
    file_handler = logging.FileHandler('training_celebahq.log')
    ...
    try:
        ...
        data_dir = "D:\Wajahat Ali Khan\KHU Gangdong Hospital Data\PET1_FDG\ADNI1"
