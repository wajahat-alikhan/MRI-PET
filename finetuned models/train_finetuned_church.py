...
class DDPMTrainer:
    def __init__(
        ...
        device,
        num_epochs=50,
        save_dir='ddpm_checkpoints_church',
        ...
        self.vis_dir = self.save_dir / 'visualizations_church'
    ...
    def save_checkpoint(self, epoch, is_best=False):
        ...
        checkpoint_path = self.save_dir / f'checkpoint_epoch_{epoch}_church.pt'
        torch.save(checkpoint, checkpoint_path)
        
        if is_best:
            best_model_path = self.save_dir / 'best_model_church.pt'
    ...
    def create_prediction_visualization(self, mri, pet, pred_pets, epoch):
        try:
            ...
            fig.suptitle(f'Predictions at Epoch {epoch} (church)', fontsize=16, y=0.92)
            ...
            plt.tight_layout()
            save_path = self.vis_dir / f'predictions_epoch_{epoch:04d}_church.png'
    ...
    def plot_training_curves(self):
        ...
        try:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title('Training Loss Over Time (church)')
            plt.grid(True)
            plt.legend()
            ...
            plt.title('Image Quality Metrics Over Time (church)')
            plt.grid(True)
            plt.legend()
            
            plt.tight_layout()
            save_path = self.save_dir / 'training_metrics_church.png'
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
            plt.close()
    ...
    def create_final_evaluation_visualization(self, num_samples=10):
        ...
        """
        save_path = self.save_dir / 'final_evaluation_results_church.png'
        self.model.eval()
        
        # Create figure with 3 columns (MRI, Real PET, Predicted PET) and num_samples rows
        fig, axes = plt.subplots(num_samples, 3, figsize=(15, 5 * num_samples))
        fig.suptitle('Model Evaluation: MRI to PET Translation (church)', fontsize=16, y=0.92)
...
def setup_training(
    ...
    try:
        ...
        try:
            pipe = DDPMPipeline.from_pretrained("google/ddpm-church-256")
            pipe.to(device)
...
if __name__ == "__main__":
    ...
    file_handler = logging.FileHandler('training_church.log')
